...
const StudRegister = () =>{
    ...
    const [student, setStudent] = useState({
      ...
      const {name,email,collegeId,mobile} = student
    ...
    const onSubmit = async (e) =>
    {
       ...
       await axios.post("http://localhost:7777/addRegister",student);
       navigate("/stud")
       setDis2("blur-lg")
       setDis1("visible")
    ...
    const sms = () =>
    {
         fromPhoneNumber = 7758937762;
         toPhoneNumber = mobile
         message='Jai Shree Ram...!!'

         axios.post("http://localhost:8888/send",{fromPhoneNumber,toPhoneNumber,message})
    }
  ...
  return (
  <>
    <div className={`relative bg-gray-200 min-h-screen min-w-screen flex flex-row justify-center ${dis2}`}>
        <div className='bg-white w-2/6 mb-20 mt-20 rounded-l-lg'>
            ...
            <label className='ml-10'>College Id <span className='text-red-600'>*</span></label><br/>
            <input type='text' placeholder='Enter Your College Id' name="collegeId" value={collegeId} onChange={(e)=>onInputChange(e)} className='border-2 ml-10 mt-3 w-96 h-12 pl-4 mb-5' required/><br/>
            ...
            <button className={`w-28 h-10 bg-blue-700 rounded-full text-white test-2xl hover:bg-blue-900 ml-5 ${dis}`} onClick={sms}>Verify</button><br/>
            <button className='mt-10 bg-blue-700 w-96 h-14 ml-10 text-white rounded-full text-2xl mb-10 hover:bg-blue-900' type="submit">Register</button>
            </form>
        </div>
        <div className='relative bg-Register w-2/6 mt-20 mb-20 bg-cover rounded-r-lg'>
       ...
       <div className={`absolute top-1/3 h-96 w-2/3 left-72 rounded-2xl text-center bg-gray-700 z-10 ${dis1}`}>
